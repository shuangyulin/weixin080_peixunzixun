<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="项目评价" prop="xiangmupingjia">
          <el-select v-model="ruleForm.xiangmupingjia" placeholder="请选择项目评价">
            <el-option
                v-for="(item,index) in xiangmupingjiaOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="项目评价" prop="xiangmupingjia">
	      <el-input v-model="ruleForm.xiangmupingjia"
                placeholder="项目评价" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="咨询师评价" prop="zixunshipingjia">
          <el-select v-model="ruleForm.zixunshipingjia" placeholder="请选择咨询师评价">
            <el-option
                v-for="(item,index) in zixunshipingjiaOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="咨询师评价" prop="zixunshipingjia">
	      <el-input v-model="ruleForm.zixunshipingjia"
                placeholder="咨询师评价" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="满意度" prop="manyidu">
        ...
        <div v-else>
          <el-form-item class="input" label="满意度" prop="manyidu">
            ...
            <el-col :span="24">
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.pingyu" label="评语" prop="pingyu">
                    <span v-html="ruleForm.pingyu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.pingyu = this.ruleForm.pingyu.replace(reg,'../../../ssm1x9di/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xiangmupingjia/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xiangmupingjiaCrossAddOrUpdateFlag = false;
                  this.parent.search();
