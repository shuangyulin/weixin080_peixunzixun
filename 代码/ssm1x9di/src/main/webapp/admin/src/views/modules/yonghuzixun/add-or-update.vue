<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="咨询编号" prop="zixunbianhao">
            <el-input v-model="ruleForm.zixunbianhao" 
                placeholder="咨询编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.zixunbianhao" label="咨询编号" prop="zixunbianhao">
              <el-input v-model="ruleForm.zixunbianhao" 
                placeholder="咨询编号" readonly></el-input>
          </el-form-item>
        </div>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.zixunwenti = this.ruleForm.zixunwenti.replace(reg,'../../../ssm1x9di/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yonghuzixun/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yonghuzixunCrossAddOrUpdateFlag = false;
                  this.parent.search();
