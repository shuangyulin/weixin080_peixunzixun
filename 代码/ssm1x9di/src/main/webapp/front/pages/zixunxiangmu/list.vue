	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.zixunleibie" type="text" placeholder="咨询类别" ></input>
			</view>
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"rgba(255, 255, 255, 1)"}' v-for="(product,index) in list" :key="index">
								                                <view class="uni-product-title" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"left"}'>{{product.xiangmumingcheng}}</view>
					...
					<text v-if="isAuth('zixunxiangmu','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('zixunxiangmu','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"#409EFF","borderColor":"#409EFF","borderRadius":"8rpx","color":"#fff","borderWidth":"2rpx","width":"160rpx","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' v-if="isAuth('zixunxiangmu','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.zixunleibie){
					params['zixunleibie'] = '%' + this.searchForm.zixunleibie + '%'
				}	
				...
				let res = await this.$api.list(`zixunxiangmu`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zixunxiangmu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					searchForm['xiangmumingcheng'] = '%' + this.searchForm.xiangmumingcheng + '%'
																...
																if(this.searchForm.zixunleibie){
					searchForm['zixunleibie'] = '%' + this.searchForm.zixunleibie + '%'
				}	
																																																																								if(this.searchForm.shangjiaxingming){
					searchForm['shangjiaxingming'] = '%' + this.searchForm.shangjiaxingming + '%'
				}	
																																				let res = await this.$api.list(`zixunxiangmu`, searchForm);
				// 如果是第一页数据置空
